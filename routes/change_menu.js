...
/*
A user sends a GET request to view a list of their menu items.

If user is an owner:
  The user is rendered 'change_menu' page with a JSON containing a list of their menu items
	found inside the items database.
Otherwise:
...
router.post('/', function(req, res){
  ...
  var required_param = [req.session.user_id, req.body.item_name, req.body.desc, req.body.allergens, req.body.price];
	...
	if( isUndefined(required_param) ) {
		...
		return send_to_page(req, res, "no edit");
  }
  ...
  items.find({} , "item_name item_id", function(err, docs){
    for(var i = 0; i < docs.length; i++){
      if(docs[i].item_name == data.item_name && docs[i].item_id != data.item_id){
        return send_to_page(req, res, "duplicate item");
      }
    }
    items.find( { item_id: data.item_id }, "item_id item_name desc allergens price", function(err, docs){
      items.update(docs[0], data, function(err, docs){
        if(err) { throw err; }
        return send_to_page(req, res, "changed");
      })
    }) //items.find
  })
//router.post
